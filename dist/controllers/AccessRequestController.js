...
export class AccessRequestController {
    ...
    static async getAllAccessRequests(req, res) {
        try {
    ...
    static async getAccessRequestById(req, res) {
        try {
            const { id } = req.params;
    ...
    static async createAccessRequestByCase(req, res) {
        try {
    ...
    static async updateAccessRequest(req, res) {
        try {
            const { id } = req.params;
            ...
            const accessRequest = await AccessRequest.findByIdAndUpdate(id, updates, { new: true, runValidators: true }).populate('evidenceId');
            if (!accessRequest) {
                res.status(404).json({ error: 'Access request not found' });
                return;
            }
    ...
    static async approveAccessRequest(req, res) {
        try {
            ...
            const accessRequest = await AccessRequest.findByIdAndUpdate(requestId, {
                status: 'approved',
                approvedBy,
                approvalTimestamp: new Date().toISOString()
            }, { new: true, runValidators: true }).populate('evidenceId');
            if (!accessRequest) {
                res.status(404).json({ error: 'Access request not found' });
                return;
            }
    ...
    // Deny access request
    static async denyAccessRequest(req, res) {
        try {
            const { requestId } = req.params;
            const { approvedBy } = req.body;
            const accessRequest = await AccessRequest.findByIdAndUpdate(requestId, {
    ...
    static async getAccessRequestsByUser(req, res) {
        try {
    ...
    static async getPendingAccessRequests(req, res) {
        try {
    ...
    static async deleteAccessRequest(req, res) {
        try {
            const { id } = req.params;
